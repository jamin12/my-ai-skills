...
개인용 AI 스킬 저장소. 여러 프로젝트·여러 컴퓨터에서 같은 스킬을 심볼릭 링크 기반으로 공유한다. Claude와 Codex에서 공용으로 쓸 수 있게 구성했고, 외부 CLI 의존 없이 `npx` 로 직접 실행되는 얇은 Node.js 스크립트(`bin/my-skills.mjs`) 하나로 설치/업데이트를 관리한다.

## 동작 원리

1. `npx github:jamin12/my-ai-skills install ...` 실행
2. `bin/my-skills.mjs` 가 `~/.my-ai-skills` 에 repo 를 clone (또는 기존 clone 을 `git pull`)
3. `~/.my-ai-skills/skills/**/<name>` 를 재귀 탐색해서, 설치 시에는 선택한 대상 디렉토리(`~/.claude/skills`, `./.claude/skills`, `~/.agents/skills`, `./.agents/skills`, `~/.codex/skills`) 아래에 `<name>` 으로 평탄하게 심볼릭 링크 생성
4. 이후 스킬 수정은 `~/.my-ai-skills` 에서만 하고 `git push`
5. 다른 컴퓨터에서는 `npx github:jamin12/my-ai-skills update` 한 줄로 모든 설치가 즉시 최신 상태
...
```bash
# 전부 설치
npx -y github:jamin12/my-ai-skills install

# 일부만 설치
npx -y github:jamin12/my-ai-skills install spec-setup cmux-worktree
```

`~/.claude/skills/` 아래에 심볼릭 링크가 생성된다. 원본은 `~/.my-ai-skills/skills/` 아래의 실제 스킬 디렉토리다.

인수 없이 `install` 을 실행하면 인터랙티브 모드가 열리고, 먼저 카테고리(`backend`, `general` 등)를 고른 뒤 그 안의 스킬이 기본 선택된 상태로 열린다. 그대로 `Enter` 하면 카테고리 전체를 한 번에 설치할 수 있고, 필요하면 일부만 해제하면 된다.

### Codex 사용자 전역에 설치

```bash
# Codex 사용자 스킬 경로 (~/.agents/skills)
npx -y github:jamin12/my-ai-skills install --codex --global

# backend 번들을 한 번에 설치
npx -y github:jamin12/my-ai-skills install --codex --global @backend
```
...
```bash
npx -y github:jamin12/my-ai-skills install --target codex-home
```
...
# 설치된 스킬 확인
npx -y github:jamin12/my-ai-skills list
npx -y github:jamin12/my-ai-skills list --project
npx -y github:jamin12/my-ai-skills list --codex --global
npx -y github:jamin12/my-ai-skills list --codex --project
npx -y github:jamin12/my-ai-skills list --target codex-home

# 제거
npx -y github:jamin12/my-ai-skills remove spec-setup
npx -y github:jamin12/my-ai-skills remove --project usecase
npx -y github:jamin12/my-ai-skills remove --codex --project usecase
npx -y github:jamin12/my-ai-skills remove --codex --project @backend
```

`update` 는 `~/.my-ai-skills` 를 `git pull` 한 번 하는 것. 심볼릭 링크가 새 파일 내용을 자동으로 가리킨다.
...
# backend 묶음 설치
npx -y github:jamin12/my-ai-skills install --codex --project @backend

# 개별 스킬과 섞어서 설치 가능
npx -y github:jamin12/my-ai-skills install --codex --project @backend spec-setup
```
...
- **수정은 `~/.my-ai-skills` 에서만 한다.** 설치된 각 프로젝트의 `.claude/skills/`, `.agents/skills/`, `~/.agents/skills/`, `~/.codex/skills/` 는 모두 심볼릭 링크이므로, 그 경로에서 편집하면 원본도 바뀐다. 단 편집 후 반드시 `~/.my-ai-skills` 에서 `git add/commit/push` 해야 다른 컴퓨터에 전파된다.
