...
- **OutPort 반환**: 기본적으로 **도메인 모델** 반환. 도메인 모델로 표현 불가 시(조인/집계) `Projection` DTO 사용.
...
| Projection | OutPort에서 도메인 모델로 표현 불가한 반환값(조인/집계) | `{의미}Projection` | `VotableDebateProjection` |
...
└── Projection.kt   # OutPort 반환용 (도메인 모델로 표현 불가한 조인/집계, 있는 경우만)
```
